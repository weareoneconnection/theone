  ...
  const oneAiResult = await runOneAI<unknown>({
    ...
    input: {
      ...
      instruction: [
        ...
        'NEVER derive a URL, hostname, repository, or file path from identifiers found inside pasted code, logs, stack traces, error messages, or quoted text. Module names, API calls, and package names (for example fs.mkdtemp, axios.get, react-dom) are NOT web addresses. An external target is valid only when the user stated it as a target.',
        'When the user pastes code, logs, a document, or any block of material and asks to analyze, explain, review, summarize, or debug it, that material IS the source. Answer directly from it with needsWorker=false and finalAnswerReady=true. Do not create browser, search, or fetch tasks to look for it elsewhere.',
        ...
        'For X/Twitter posts, generate publication-ready content under 260 characters unless the user explicitly asks for a thread.',
	        ...
	        'For repository mutation requests, prefer the AGENT route: one code.patch.apply step whose input carries workspacePath and objective (the complete task description in the user language, including what to change and how to verify). OneClaw runs a self-hosted coding agent that explores, edits, runs approved tests, and verifies inside one governed, git-snapshotted run, returning a diff receipt with token usage.',
	        'An agent-route task must contain at most two steps: optionally code.workspace.status for inspection, then code.patch.apply {workspacePath, objective}. Do NOT add code.diff.prepare, code.test.run, or code.verify steps — the agent run performs and reports these internally; extra steps only multiply approval prompts.',
	        'Use direct-write mode (code.patch.apply with files[] carrying full content) only when the user explicitly provided the exact file contents to write. Never pre-generate file contents for agent-route tasks; path-only files[] lists are allowed as hints.',
	        'code.patch.apply is always manual approval gated and high risk; one approval covers the whole agent run.',
	        'If — and only if — the user explicitly asks to open a pull request, submit a PR, deliver, or ship the change (e.g. "开 PR", "提交 PR", "帮我发个 PR", "deliver as a PR"), set the code.patch.apply step input.deliver=true. After a verified run the agent commits onto a fresh branch, pushes it, and opens a pull request; it never pushes a protected branch and never merges. Do NOT set deliver for a normal fix/edit request — default is a local diff the user reviews.',
	        'Use code.workspace.status for read-only workspace inspection, repository status, and project health checks.',
	        'Use code.patch.rollback when an applied patch has a rollbackToken and the user wants the change undone. Rollback is always approval gated.',
	        ...
	        'Use code.pr.create to prepare a pull request package only after a verified agent run. It is approval gated and must not claim a live PR exists without a provider receipt.',
