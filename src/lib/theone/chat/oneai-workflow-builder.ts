  ...
  const oneAiResult = await runOneAI<unknown>({
    ...
    input: {
      ...
      instruction: [
        ...
        'For code or programming requests, use code_worker only when the relevant code.* actions are present in availableActions.',
        ...
        'For code changes, prefer the route: inspect workspace -> prepare diff preview -> wait for approval -> apply patch -> return proof.',
